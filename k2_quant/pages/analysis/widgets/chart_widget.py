...
class ChartWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.indicator_panes = {}  # {indicator_name: plot_widget}
    ...
    def load_data(self, data: pd.DataFrame):
        ...
        date_columns = ['date', 'datetime', 'date_time', 'timestamp', 'date_time_market']
        ...
        self.x_values = np.arange(len(data))
        
        # Setup x-axis formatting
        self.setup_x_axis()
        
        k2_logger.info(f"Data loaded: {len(data)} records", "CHART")
    ...
    def setup_x_axis(self):
        ...
        first_date = str(self.data.iloc[0][self.date_column])
        has_time = len(first_date) > 10 and ':' in first_date
        
        # Create tick labels
        num_ticks = min(10, len(self.data))
        tick_spacing = max(1, len(self.data) // num_ticks)
        ...
        for i in range(0, len(self.data), tick_spacing):
            ...
            if pd.notna(date_val):
                ...
                if has_time:
                    # Show date at day boundaries, time otherwise
                    if i == 0 or str(self.data.iloc[i-1][self.date_column])[:10] != date_str[:10]:
                        label = date_str[:10]  # Show date
                    else:
                        label = date_str[11:16]  # Show time HH:MM
                else:
                    label = date_str[:10]  # Just date
                
                ticks.append((i, label))
        
        axis.setTicks([ticks])
    
    def add_line(self, column_name: str, data: pd.Series = None, color: str = None):
        """Add a line to the main chart"""
        if column_name in self.active_lines:
            return  # Already displayed
        
        if data is None and self.data is not None:
            if column_name.lower() in self.data.columns:
                data = self.data[column_name.lower()]
            else:
                k2_logger.warning(f"Column {column_name} not found", "CHART")
                return
        ...
        plot_item = self.main_plot.plot(self.x_values[:len(data)], data.values, 
                                       pen=pen, name=column_name)
        
        self.active_lines[column_name] = plot_item
        k2_logger.info(f"Added line: {column_name}", "CHART")
    
    def remove_line(self, column_name: str):
    ...
    def add_indicator_pane(self, indicator_name: str, data: pd.Series, 
                          chart_type: str = 'line'):
        ...
        if chart_type == 'line':
            ...
            # Bar chart for volume-like indicators
            bargraph = pg.BarGraphItem(x=self.x_values[:len(data)], 
                                      height=data.values, width=0.8, 
                                      brush='#4a4')
    ...
    def remove_indicator_pane(self, indicator_name: str):
    ...
    def on_mouse_clicked(self, evt):
        ...
        if self.main_plot.sceneBoundingRect().contains(pos):
            ...
            if self.drawing_mode == 'horizontal':
                # Add horizontal line
                line = pg.InfiniteLine(pos=mousePoint.y(), angle=0, 
                ...
                # Add vertical line
                line = pg.InfiniteLine(pos=mousePoint.x(), angle=90, 
                                      pen=pg.mkPen('#4aa', width=2))
                self.main_plot.addItem(line)
                self.drawings.append(('vline', line))
            
            # TODO: Implement trend line and text annotations
    ...
    def add_projection(self, name: str, projection_data: pd.Series):
        ...
        plot_item = self.main_plot.plot(x_values, projection_data.values, 
                                       pen=pen, name=f"{name} (Projection)")
        self.active_lines[f"{name}_projection"] = plot_item
    
    def remove_projection(self, name: str):
