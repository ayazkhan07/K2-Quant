...
Contains AI chat interface with streaming responses for strategy development.
"""
...
class RightPaneWidget(QFrame):
    ...
    # Signals
    code_generated = pyqtSignal(str, str)  # code, description
    strategy_saved = pyqtSignal(str, str)  # name, code
    ...
    def init_ui(self):
        ...
        self.streaming_indicator.setMaximum(0)  # Indeterminate
    ...
    def create_header(self) -> QWidget:
        ...
        label = QLabel("CONVERSATIONAL AI")
    ...
    def create_model_selector(self) -> QWidget:
        ...
        self.provider_selector.addItems(["OpenAI", "Anthropic"])
    ...
    def set_data_context(self, data: Any, metadata: Dict[str, Any]):
        ...
        # Show in chat
        self.add_system_message(f"📊 Loaded {metadata.get('symbol', 'Unknown')} data with {self.current_context['data_shape'][0] if isinstance(self.current_context['data_shape'], tuple) else self.current_context['data_shape']} records")
    
    def send_quick_prompt(self, prompt_type: str):
    ...
    def start_streaming(self, message: str):
        ...
        self.add_ai_message("")  # Empty message to be filled by streaming
    ...
    def process_code_block(self, text: str):
